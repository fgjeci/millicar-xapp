...
class IntegerPSO(DiscreteSwarmOptimizer):
    ...
    def closest(self, matrix, listElem):
        return np.vectorize(lambda matrixElem: min(listElem, key=lambda x: abs(x-matrixElem)))(matrix)


    def _compute_position(self, swarm):
        """Update the position matrix of the swarm

        This computes the next position in a discrete swarm. It takes the velocity vector and normalizes
        by the max value in the velocity vector to obtain values of velocity in the interval [0, 1];
        The velocity vector shall indicate the probability of doing the swap in each of its position

                                                        max
        v       ...     0.15    0.22    0.63    0.94    0.51 ...
        ...
        # _norm_velocity = swarm.velocity .T / np.max(swarm.velocity, axis=1)
        # _argmax_velocity = np.argmax(_norm_velocity)

        # _gbest_pos_index = swarm.best_pos[_argmax_velocity]
        #
        # _x_swap_max, _ = np.where(swarm.position == _gbest_pos_index) # integers
        #
        _position = swarm.position.copy()
        #
        # _position[_x_swap_max], _position[_gbest_pos_index] = _position[_gbest_pos_index], _position[_x_swap_max]
        #
        # return _position
        _position = _position + swarm.velocity
        # before returning we need to reset in the accepted base
        _position = self.closest(_position, [0, 1, 2, 3, 4, 5, 6])
